...
#触发器
DELIMITER //
CREATE TRIGGER baoxian_copy
AFTER DELETE ON baoxian
FOR EACH ROW
BEGIN
INSERT INTO baoxianscopy VALUES(old.baoxianid,old.baoxianname,old.feiyong);
END
//DELIMITER

#查询保险回收站
SELECT * FROM baoxianscopy;

#删除保险回收站
DELETE FROM baoxianscopy
...
#触发器
DELIMITER //
CREATE TRIGGER hangkong_copy
AFTER INSERT ON hangkong
FOR EACH ROW
BEGIN
INSERT INTO hangkongscopy 
VALUES(new.hangkongid,new.hangkongname,new.hangkongnumber,new.hangkongcf,new.hangkongfw,new.hangkongrq,new.hangkongfeiyong);
END
//DELIMITER

#添加航空表数据
INSERT INTO hangkong () 
VALUES (10,'东方航空','CZ6470','广州白云国际机场','重庆江津机场','2020-10-21-10:20','￥620.00');

#查询航空添加表
SELECT * FROM hangkongscopy;

#删除航空添加表
DROP TABLE hangkongscopy;
...
INSERT INTO gonglue (jingdiandd,jiudianname,huochenumber,dabakw,youlunname,hangkongname,yulename) 
VALUE ('上海','上海丽景精选酒店','G100','上海客运车站',NULL,'中国国航','上海Tiffany蓝泳池');
INSERT INTO gonglue (jingdiandd,jiudianname,huochenumber,dabakw,youlunname,hangkongname,yulename) 
...
INSERT INTO gonglue (jingdiandd,jiudianname,huochenumber,dabakw,youlunname,hangkongname,yulename) 
VALUE ('湖北','宜昌市伍家岗区琪琳宾馆','G1032','宜昌客运车站','世纪系列游轮','南方航空','湖北武当太极湖水上游乐');
INSERT INTO gonglue (jingdiandd,jiudianname,huochenumber,dabakw,youlunname,hangkongname,yulename) 
...
#创建航空视图
CREATE VIEW Consumer_aviation 
AS SELECT gonglue.gonglueid,consumer.consumername,hangkong.hangkongname,jingqu.jingdiandd,jiudian.jiudianname,yule.yulename,baoxian.baoxianname 
FROM gonglue,consumer,hangkong,jingqu,jiudian,yule,baoxian 
WHERE gonglue.hangkongname=hangkong.hangkongname
AND gonglue.jingdiandd=jingqu.jingdiandd
AND gonglue.jiudianname=jiudian.jiudianname
AND gonglue.yulename=yule.yulename
AND consumer.baoxianname=baoxian.baoxianname

#查询航空攻略视图
SELECT*FROM Consumer_aviation

#删除航空攻略视图
DROP VIEW Consumer_aviation

#创建客车视图
CREATE VIEW consumer_bus 
AS SELECT gonglue.gonglueid,consumer.consumername,daba.dabakw,jingqu.jingdiandd,jiudian.jiudianname,yule.yulename,baoxian.baoxianname 
FROM gonglue,consumer,daba,jingqu,jiudian,yule,baoxian 
WHERE gonglue.dabakw=daba.dabakw
AND gonglue.jingdiandd=jingqu.jingdiandd
AND gonglue.jiudianname=jiudian.jiudianname
AND gonglue.yulename=yule.yulename
AND consumer.baoxianname=baoxian.baoxianname
...
#查询客车攻略视图
SELECT*FROM Consumer_bus

#删除客车攻略视图
DROP VIEW Consumer_bus

#创建火车视图
CREATE VIEW Consumer_train 
AS SELECT gonglue.gonglueid,consumer.consumername,huoche.huochenumber,jingqu.jingdiandd,jiudian.jiudianname,yule.yulename,baoxian.baoxianname 
FROM gonglue,consumer,huoche,jingqu,jiudian,yule,baoxian 
WHERE gonglue.huochenumber=huoche.huochenumber
AND gonglue.jingdiandd=jingqu.jingdiandd
AND gonglue.jiudianname=jiudian.jiudianname
AND gonglue.yulename=yule.yulename
AND consumer.baoxianname=baoxian.baoxianname
...
#查询火车攻略视图
SELECT*FROM Consumer_train

#删除火车攻略视图
DROP VIEW Consumer_train

#创建娱乐表
CREATE TABLE yule(
yuleid INT,
yulename VARCHAR(20) PRIMARY KEY,
yuledizhi VARCHAR(50),
yulesslx VARCHAR(60)
);
...
UPDATE consumer SET spend=spend-5000 
WHERE consumername='张春华';
UPDATE income SET money=money+5000 
WHERE gsname='天宇财务';
...
#查询性别为女的人数
SELECT COUNT(gender)FROM consumer WHERE gender LIKE'女%'
#存储过程
DELIMITER //
CREATE PROCEDURE proc_selectCountconsumergender(IN c CHAR(2))
BEGIN
SELECT COUNT(gender) FROM consumer 
GROUP BY consumer.gender
HAVING consumer.gender=c;
END
//
DELIMITER

CALL proc_selectCountconsumergender('女');

#查询消费表总人数
SELECT COUNT(consumername)FROM consumer WHERE consumerid>'0'
#存储过程
DELIMITER //
CREATE PROCEDURE proc_selectAllCountconsumername(OUT zong INT)
BEGIN
SELECT COUNT(consumerid) INTO zong FROM consumer ;
END
//
DELIMITER
SET @zong=0;

CALL proc_selectAllCountconsumername(@zong);
SELECT @zong;
